...
const DB = {
 ...
 async addUser(user) {
  console.log('👤 DB.addUser called:', user);
  ...
  const prefix = user.role === 'admin' ? 'ADM' : user.role === 'security' ? 'SEC' : user.role === 'coordinator' ? 'COO' : 'STU';
  const userCount = await db.users.where('role').equals(user.role).count();
  const idNum = userCount + 1;
  
  // ✅ AUTO-APPROVE LOGIC:
  // - Admin: auto-approved ✅
  // - Security / Coordinator: auto-approved ✅
  // - Student: pending approval ⏳
  const role = (user.role || 'student').toLowerCase();
  const autoApprove = (role === 'admin' || role === 'security' || role === 'coordinator');
  ...
  console.log('💾 Saving user with approved:', newUser.approved);
  
  await db.users.add(newUser);
  
  // AUDIT LOG
  await logAudit('user', 'USER_CREATED', newUser.id, 
    `User ${newUser.name} (${newUser.email}) created with role: ${newUser.role} (approved: ${newUser.approved})`);
  ...
  determineIncidentPriority(type, description = '') {
  ...
  incident.priority = this.determineIncidentPriority(incident.type, incident.description);
...
async updateIncident(id, updates) {
  ...
  async login(email, password, role) {
    ...
    // If role is provided, validate it. If not provided (undefined), skip validation
  ...
  async getStats() {
    ...
    return {
      ...
      activeOfficers: users.filter(u => (u.role === 'security' || u.role === 'coordinator') && u.active).length,
